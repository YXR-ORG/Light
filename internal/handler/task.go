...
func (h *TaskHandler) StreamTask(req StreamTaskRequest) error {
	...
	tools, finishGoalTool, specTool := eino.BuildTaskTools(ctx, req.WorkDir, func(stepType, content, cmd, confirmID string) {
		runtime.EventsEmit(h.ctx, "task:step", eino.TaskStep{
			ConvID: req.ConversationID, Type: stepType, Content: content, Cmd: cmd, ConfirmID: confirmID,
		})
	}, planEnabled, req.Goal, req.Workflow)
	// 找出 BashTool 引用
	for _, t := range tools {
	...
	collectArtifact := func(toolResult string) {
		for _, a := range eino.ParseArtifacts(toolResult) {
			...
			// plan/requirement/review 始终保留最新；file 则 write 优先覆盖 read。
			if !ok {
				artifactMap[key] = a
				artifactOrder = append(artifactOrder, key)
			} else if a.Type == "plan" || a.Type == "requirement" || a.Type == "review" {
				artifactMap[key] = a
