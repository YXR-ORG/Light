...
└──────────────────────┬──────────────────────────────────────┘
                       │ 检索
┌──────────────────────▼──────────────────────────────────────┐
│              四路融合检索 + RRF 重排序 Store.Search()         │
│                                                              │
│  阶段一：摘要层预过滤（定位相关文档，命中文档 ×1.2 加权）     │
│                                                              │
...
│    多路命中累加得分 → 按 RRF 分降序 → top-k chunks           │
└──────────────────────┬──────────────────────────────────────┘
                       │ 对话
┌──────────────────────▼──────────────────────────────────────┐
...
**决策7：RRF 替代简单去重追加作为四路融合策略**

原方案把四路结果顺序追加（先来先得），FTS5 第1名和向量第1名地位完全平等，排列顺序取决于哪路先检索完成，而非真正的相关性。

RRF（Reciprocal Rank Fusion，k=60）让每路结果携带排名参与融合：
- 同一 chunk 在多路出现时，分数累加（多路认同 = 置信度更高）
- 只在单路出现的 chunk 得分较低（单一信号置信度弱）
- 每路取 topK×2 候选，融合池更充分
- 无需额外模型，纯数学，零延迟

相比 Reranker（cross-encoder 神经网络重排），RRF 无需增加 ~100MB 的第二个 ONNX 模型，延迟为零，且在个人知识库的小规模文档场景下效果差异不显著。
...
- 快速定位相关文档（两阶段检索第一阶段）
- 重建索引时提供文档上下文
- 未来：搜索时先匹配摘要层，再搜 chunk 层（见 TODO-KB-3）
