...
- **四种对话模式**：
  ...
  - **工作流模式**：结构化的「需求→设计→计划→编码→验收」开发工作流
- **流式输出**：打字机效果实时展示 AI 回复，支持思考链（Thinking）展示
- **联网搜索**：集成 Tavily / Exa / Brave / SearXNG 等搜索引擎，AI 可自动调用搜索获取最新信息
- **本地知识库**：上传文档（TXT/MD/PDF/DOCX/Excel），四路融合检索（FTS5 + 向量 + 摘要 + LIKE）+ 同义词归一化
...
- **会话分叉**：在任意 AI 回复处一键分叉，复制到该回复为止的历史创建新会话，原会话不受影响；分叉会话在列表中以角标标识来源
- **上下文压缩**：任务模式自动管理上下文大小，reduction 机械裁剪大工具输出 + summarization 语义压缩长对话，避免 token 爆炸
- **Goal 模式 + 独立验收**：任务模式下设定目标与验收标准，Agent 自主执行；`finish_goal` 后由独立裁判对照标准验收，不通过则打回继续，支持轮数上限
- **工作流独立审查**：编码完成后起全新审查 Agent（只读工具 + 变更清单），不带主 Agent 推理历史，客观审查后产出 review 卡片
- **用量统计**：自动记录每次 LLM 调用的 token 与费用估算；设置页独立「用量统计」面板，按天 / 服务商 / 模式 / 模型汇总；对话头部显示会话累计用量
...
1. 任务模式下点击「目标」，填写目标描述
2. 可选填写验收标准（每行一条，如「测试全部通过」）和最多打回次数（默认 5）
3. Agent 自主执行，调用 `finish_goal` 后由独立裁判验收
4. 不通过则打回继续修改，达到上限后强制结束并总结

**工作流模式**：

1. 输入框切换为「工作流」模式，填写需求描述
2. Agent 按 需求规格 → 方案设计 → 执行计划 → 编码实现 → 完成确认 顺序推进
3. 完成后自动发起独立审查 Agent（只读工具核实）+ 用户验收标准核对
4. 审查 / 验收结果以卡片展示，不通过会打回继续改

**用量统计**：

1. 打开 **设置 → 用量统计**
2. 切换今天 / 近 7 天 / 近 30 天 / 全部，查看 token 与费用估算
3. 可按服务商、模式、模型拆分；对话头部也会显示本会话累计用量

> 危险命令（如 `rm -rf`）执行前会弹窗确认，黑名单可在 **设置 → 通用设置** 中配置。
> 默认不主动写文件，仅当你明确要求「保存 / 导出 / 生成文件」时才落地。
> 费用为内置模型单价估算，仅供参考；部分服务商若不返回 usage，则对应记录为 0。
...
### v2.1.0
- **Goal 独立验收**：目标模式支持用户自定义验收标准与打回轮数上限
  - `finish_goal` 后由独立裁判对照标准验收，不通过则反馈打回继续
  - 达到轮数上限后强制结束并生成总结
  - 目标输入区新增验收标准（每行一条）与最多打回次数配置
- **工作流独立第二代理审查**：替换同上下文自审，起全新审查 Agent
  - 只喂需求 / 验收标准 / 变更文件清单，不带主 Agent 推理历史
  - 绑定只读工具（read_file / list_dir / 受限 bash），可自行核实
  - 审查失败同样打回主 Agent 继续修改
- **用量统计**：采集 → 落库 → 展示全链路
  - Message 记录 token 字段，新增 `token_usages` 汇总表与内置模型单价
  - chat / task 流式读取服务商返回的 usage 并持久化
  - 设置页独立「用量统计」菜单：按天 / 服务商 / 模式 / 模型汇总
  - 对话头部显示会话累计 tokens（任务模式含实时累计）；消息底部显示本条 tokens

### v2.0.0
- **知识库查询预处理 + 同义词归一化**：引入中文分词与口语→标准词映射，提升召回质量
  - query 侧 gse 分词 + 停用词清洗 + 同义词扩展
  - 向量检索保留原 query，FTS5 / 摘要用归一化 query
  - 归一化映射透传给 LLM，避免「检索到了但模型拒绝作答」

### v1.9.0
- **工作流模式**：结构化的"需求→设计→计划→编码→验收"开发工作流
  - 新增第四种对话模式 workflow，与 task/goal 互斥
  - Agent 先调用 submit_spec 提交需求规格和验收标准（产出 requirement 卡片）
  - 按 5 阶段顺序自主执行：需求分析→方案设计→执行计划→编码实现→完成确认
  - 编码完成后自动发起独立验收审查轮次，对照验收标准逐项 pass/fail 检查（产出 review 卡片）
  - 复用 goal 模式的 autoApprove + finish_goal + 持续执行逻辑
  - 产物仅 artifact 卡片展示，不落盘文件

### v1.8.0
- **Goal 模式**：任务模式下设定目标，Agent 自主持续执行直到目标达成
  - Agent 调用 finish_goal 工具显式结束并附完成总结
  - 普通危险命令自动放行，仅极危险命令（rm -rf /、sudo、mkfs 等）需用户确认
  - 目标注入 system prompt，引导 Agent 不中途停下来问用户
  - 修复 BashConfirmDialog 事件名不匹配导致危险命令确认弹窗从未显示的 BUG

### v1.7.0
- **会话分叉**：在任意 AI 回复处一键分叉，复制到该回复为止的历史创建新会话，原会话不受影响
  - 分叉会话继承源会话的模型、智能体、模式等配置
  - 重生成版本仅复制当前查看版本，新会话从干净状态开始
  - 分叉会话在列表中以分支角标标识来源
  - task 模式不支持分叉（因工作目录隔离复杂，暂不开放）
...
- 保持 task 正文流式输出和现有 plan 行为不变，避免影响 Agent 执行链路

### v1.5.2
- 修复历史 task 消息中的执行计划分桶：`plan` 继续按计划卡片展示，不再误计入「本次涉及的文件」
- 增加 artifact 分桶回归测试，明确 `plan` / `file` / 其他产物的 UI 区域边界
- 补充设计文档中的数据库实体关系（主库 `chat.db` 与知识库 `kb.db`）
