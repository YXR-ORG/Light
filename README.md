...
- **Goal 模式**：任务模式下设定目标，Agent 自主持续执行直到目标达成（中途不打断用户）；普通危险命令自动放行，仅极危险命令需确认；Agent 调用 finish_goal 显式结束并总结成果
- **工作流模式**：结构化的"需求→设计→计划→编码→验收"开发工作流；Agent 先提交需求规格和验收标准，再按 5 阶段顺序自主执行；编码完成后自动发起独立验收审查，对照验收标准逐项检查
